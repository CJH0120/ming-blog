          ...
          {data.prev && (
            <Link href={`/${data.prev.id}`} className="flex  gap-2    rounded-lg	sm:w-[320px] border p-2 w-full flex justify-between	 items-center ">
              ...
              <div className="w-fill flex  justify-center	 items-center flex-col">
                ...
                <p className="text-ellipsis overflow-hidden w-full h-[24px] font-bold  sm:w-[230px] sm:whitespace-nowrap	  ">{data.prev.title}</p>
              </div>
            </Link>
          )}
        </div>
...
export const generateMetadata = async ({ params }: any): Promise<Metadata> => {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: data.thumbnail, height: 600, width: 800 }],
    },
  }
}
type Props = {
